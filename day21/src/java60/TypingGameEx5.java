...
class TypingGameEx5 extends Frame {
	...
	public boolean levelUpCheck() {
		/*
		 * 1. 현재 레벨에 맞는 Level인스턴스를 얻어온다.(getLevel()사용) 2. 현재 점수(score)가 다음레벨로 넘어갈
		 * 점수(levelUpScore)가 되는지의 여부를 판단해서 true 또는 false를 반환한다.
		 */
		Level level = getLevel(curLevel);
		return score == level.levelUpScore;
	}
	...
	public synchronized void levelUp() {
		 ...
		 * 2. curLevel의 값을 증가시키고, curLevel의 * Level인스턴스를 얻어온다.(getLevel()사용) 
	...
	public void showLevel(int level) {
		...
		showTitle(tmp, 2 * 1000); // 1초간 title을 보여준다.
	}
	...
	class WordGenerator extends Thread {
		public void run() {
			while (isPlaying) {
				...
				/*
				 * 1. 10번에 한번 꼴로 바이러스를 생성한다.(Math.random()사용) boolean isVirus = (이곳에 알맞은 수식을
				 * 넣으세요.);
				 */
