...
/*
 * [문제5] dir만 입력하면 모든 파일과 디렉토리를, dir *ex?.* 와 같이 패턴을 입력하면 
...
public class ConsoleEx5 {
	...
	static {
		try {
			curDir = new File(System.getProperty("user.dir") + "\\src\\java60");
		} catch (Exception e) {
		}
	}
	...
	public static void dir() {
		...
		case 1: // dir만 입력한 경우 현재 디렉토리의 모든 파일과 디렉토리를 보여준다.
			...
			for (int i = 0; i < curDir.listFiles().length; i++) {
				if (curDir.listFiles()[i].isDirectory()) {
		...
		case 2: // dir과 패턴을 같이 입력한 경우, 예를 들면 dir *.class
			pattern = argArr[1];
			pattern = pattern.toLowerCase(); // 패턴에서 대소문자를 구별하지 않도록 소문자로 변경한다.
...
//			pattern = pattern.replace(, )
			...
			for (int i = 0; i < curDir.listFiles().length; i++) {
				Matcher m = p.matcher(curDir.listFiles()[i].getName());
				if (curDir.listFiles()[i].isDirectory()) {
					if(m.matches())
					System.out.println("[" + curDir.listFiles()[i].getName() + "]");
				} else {
